...
#
# Its target is a file in the coverage directory named after the
# app. Its prerequesites are the application's main.py and its test file.
#
# By customizing the name of the coverage file to match the name of
# the app, Make can skip running tests if nothing has changed. This is also
# why the coverage file is deleted if the tests fail.
#
# This rule is meant to be called from a rule named after the app, not
# invoked directly.
%.cov: apps/%/main.py apps/%/test_main.py
...
# Test a single app.
#
# This rule is a shortcut for the test coverage pattern rule. It
